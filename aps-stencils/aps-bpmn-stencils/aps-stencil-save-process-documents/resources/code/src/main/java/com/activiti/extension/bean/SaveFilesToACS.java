...
public class SaveFilesToACS implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws JsonParseException, JsonMappingException, IOException{
		...
		System.out.println("Folder NodeId >>> "+nodeId);
		...
		if(relatedContentList!=null) {
			for (RelatedContent relatedContent : relatedContentList) {	
				...
				Map<String, Object> uploadResponseMap = new HashMap<String, Object>();
				uploadResponseMap = objectMapper.readValue(uploadResponse, new TypeReference<Map<String, Object>>() {
				});
				String uploadedDocNodeId = (String) ((Map<String, Object>) uploadResponseMap.get("entry")).get("id");
				System.out.println("Uploaded Document NodeId >>> "+uploadedDocNodeId);
				execution.setVariable("uploadedDocNodeId", uploadedDocNodeId);

				execution.setVariable("uploadedDocNodeIdList", ['123', '456', '789']);
