 ...
 */
@Component("basicAuthApiDelegate")
public class BasicAuthApiDelegate implements JavaDelegate {
...
//    private EndPointService endPointService;
    ...
    private static final Logger logger = LoggerFactory.getLogger(BasicAuthApiDelegate.class);
    ...
    public void execute(DelegateExecution execution) throws Exception {
        ...
        String url = (String) getInputVariableExpressionValue(execution, apiUrlExpression);
        String method = (String) getInputVariableExpressionValue(execution, httpMethodExpression);
        String requestBody = (String) getInputVariableExpressionValue(execution, requestBodyExpression);
        String headersJson = (String) getInputVariableExpressionValue(execution, headersExpression);
        int timeout = getIntVariable(execution, (String) getInputVariableExpressionValue(execution, timeOutExpression), 30000);

        String responseCodeVariable = (String) getOutputVariableExpressionValue(execution, responseCodeExpression);
        String responseMessageVariable = (String) getOutputVariableExpressionValue(execution, responseMessageExpression);
        String responseBodyVariable = (String) getOutputVariableExpressionValue(execution, responseBodyExpression);
        String apiSuccessVariable = (String) getOutputVariableExpressionValue(execution, apiSuccessExpression);
...
//            headersJson = buildRequestHeader(headersJson);
    ...
    private String getInputVariableExpressionValue(DelegateExecution execution, Expression field) {
        if(field != null){
            ExpressionManager expressionManager = Context.getProcessEngineConfiguration().getExpressionManager();
            Expression expression = expressionManager.createExpression(field.getExpressionText());
//            System.out.println(expression.getValue(execution).toString());
    ...
    private String getOutputVariableExpressionValue(DelegateExecution execution, Expression field) {
        if(field != null){
            String fieldText = removeSpecialChars(field.getExpressionText());
            ExpressionManager expressionManager = Context.getProcessEngineConfiguration().getExpressionManager();
            Expression expression = expressionManager.createExpression(fieldText);
            return expression.getValue(execution).toString();
        }else{
            return "";
        }
...
//    }
    ...
    private String removeSpecialChars(String fieldValue){
        String cleanedString = fieldValue.replaceAll("[${}]", "");
        logger.info("Cleaned String BEFORE >> ", fieldValue, " >>> Cleaned String AFTER >> ",cleanedString);
